%% ============================================================================
%  STEP 5c: Speed Control Loop
%  Creates: pmsm_speed_control.slx + test_speed.slx
%  Dynamic block positioning with bx() grid layout
%  Verified: ω=100 rad/s step, error <5 rad/s
% ============================================================================
clear; clc; close all;

fprintf('╔══════════════════════════════════════════════════════════════╗\n');
fprintf('║   STEP 5c: Speed Control Loop                              ║\n');
fprintf('╚══════════════════════════════════════════════════════════════╝\n\n');
...
fprintf('Speed PI: Kp=%.4f, Ki=%.4f, Iq_max=%.0f\n\n',Kp_speed,Ki_speed,Iq_max);
...
%  In[1]omega_ref, In[2]omega_fb -> Out[1]Iq_ref
% ======================================================================
ctrlName='pmsm_speed_control';
...
add_block('simulink/Sources/In1',[ctrlName '/omega_ref'],'Position',bx(0,0));
set_param([ctrlName '/omega_ref'],'Port','1');
add_block('simulink/Sources/In1',[ctrlName '/omega_fb'],'Position',bx(0,1));
set_param([ctrlName '/omega_fb'],'Port','2');

add_block('simulink/Math Operations/Sum',[ctrlName '/Err'],'Inputs','+-','Position',bx(1,0));
add_block('simulink/Math Operations/Gain',[ctrlName '/Kp'],'Gain','Kp_speed','Position',bx(2,0));
add_block('simulink/Continuous/Integrator',[ctrlName '/Int'],'InitialCondition','0','Position',bx(2,1));
add_block('simulink/Math Operations/Gain',[ctrlName '/Ki'],'Gain','Ki_speed','Position',bx(3,1));
add_block('simulink/Math Operations/Sum',[ctrlName '/Sum_PI'],'Inputs','++','Position',bx(3,0));
add_block('simulink/Discontinuities/Saturation',[ctrlName '/Sat'],'UpperLimit','Iq_max','LowerLimit','-Iq_max','Position',bx(4,0));

add_block('simulink/Sinks/Out1',[ctrlName '/Iq_ref'],'Position',bx(5,0));
set_param([ctrlName '/Iq_ref'],'Port','1');

add_line(ctrlName,'omega_ref/1','Err/1'); add_line(ctrlName,'omega_fb/1','Err/2');
add_line(ctrlName,'Err/1','Kp/1'); add_line(ctrlName,'Err/1','Int/1');
add_line(ctrlName,'Int/1','Ki/1');
add_line(ctrlName,'Kp/1','Sum_PI/1'); add_line(ctrlName,'Ki/1','Sum_PI/2');
add_line(ctrlName,'Sum_PI/1','Sat/1'); add_line(ctrlName,'Sat/1','Iq_ref/1');

save_system(ctrlName);
fprintf('Speed Controller saved: %s.slx\n\n',ctrlName);
...
set_param(testName,'Solver','ode4','SolverType','Fixed-step','FixedStep','1e-5','StopTime','0.2');
...
% Top-level wiring
add_block('simulink/Sources/Step',[testName '/Omega_step'],'Position',bx(0,0));
set_param([testName '/Omega_step'],'Time','0.05','Before','0','After','100');
...
add_line(testName,'Omega_step/1','Speed Controller/1');
...
add_block('simulink/Sinks/To Workspace',[testName '/W_om'],'Position',bx(6,0));
set_param([testName '/W_om'],'VariableName','om_sim','SaveFormat','Array');
add_line(testName,'Motor Plant/4','W_om/1');
...
fprintf('Simulating...\n');
try
    out=sim(testName,'StopTime','0.2','ReturnWorkspaceOutputs','on');
    om=out.get('om_sim');
    N=length(om); ss=round(N*0.9);
    om_ss=mean(om(ss:end));
    fprintf('  omega_ref=100 rad/s, omega_ss=%.4f\n',om_ss);
    if abs(om_ss-100)<5
        fprintf('\n✅ VERIFICATION PASSED\n');
    else
        fprintf('\n⚠️  Error=%.2f rad/s\n',abs(om_ss-100));
